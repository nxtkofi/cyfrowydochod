...
function InfoElement({
  ...
  return (
    <div
      className={`bg-white px-1 cursor-pointer absolute ${className ? className : ""}`}
      onClick={handleHoverCard}
    >
      <InfoIcon className="text-slate-400" />
      {showHoverCard && (
        <div
          className={`absolute z-10 bg-white rounded-md border-slate-200 border p-4 w-96 ${hoverPosition ? hoverPosition : "right-0"} flex flex-row`}
        >
